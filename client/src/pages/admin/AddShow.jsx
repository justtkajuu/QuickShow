...
const AddShow = () => {
  ...
  return nowPlayingMovies.length > 0 ? (
    <>
            ...
            >
              <div className="relative rounded-lg overflow-hidden">
                ...
                <div className="text-sm flex items-center justify-between p-2 bg-black/70 w-full absolute bottom-0 left-0">
                  <p className="flex items-center gap-1 text-gray-40-">
      ...
      {/* {Show Price Input} */}
      <div className="mt-8">
        <label className="block text-sm font-medium mb-3">Show Price</label>
      ...
      <div className="mt-6">
        ...
        <div className="inline-flex gap-5 border border-gray-600 p-1 pl-3 rounded-lg">
          ...
          <button
            ...
            className="bg-primary/80 text-white px-3 py-2 tet-sm rounded-lg hover:bg-primary cursor-pointer"
          >
            Add Time
          </button>
        </div>
                ...
                <div className="flex flex-wrap gap-2 mt-1 text-sm">
                  {times.map((time) => (
                    <div key={time} className="border border-primary px-2 py-1 flex items-center rounded">
                      <span>{time}</span>
                      <DeleteIcon onClick={() => handleRemoveTime(date, time)} width={15} className="ml-2 text-red-500 hover:textred-700 cursor-pointer"/>
                    </div>
                  ))}
                </div>
              </li>
      ...
      <button onClick={handleSubmit} disabled={addingShow} className="bg-primary text-white px-8 py-2 mt-6 rounded hover:bgprimary/90 transition-all cursor-pointer hover:scale-95">Add Show</button>
    </>
  ) : (
    <Loading />
  );
