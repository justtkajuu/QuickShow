            ...
            <span className="grid gird-cols-3 md:flex flex-wrap md:max-w-lg gap-4">
              {Object.keys(dateTime).map((date) => (
                <button onClick={()=> setSelected(date)}
                  className={`flex flex-col items-center justify-center h-14 w-14 aspect-square rounded cursor-pointer ${selceted === date ? "bg-primary text-white" : "border border-primary/70"}`}
                >
